...
export const GameHUD: React.FC = () => {
  ...
  useEffect(() => {
    const handleHUDUpdate = (e: any) => {
      ...
      if (data.health !== undefined) {
          ...
          if (data.health <= 0) setIsGameOver(true);
      }
  ...
  const handleRestart = () => {
      setIsGameOver(false);
      // Fires an event that MainLevel.ts should listen for to restart the scene
    ...
    <>
      ...
      {isGameOver && (
        <div className="absolute inset-0 z-50 flex items-center justify-center bg-black/80 backdrop-blur-sm pointer-events-auto">
            <div className="bg-[#1a1a1a] border-4 border-red-900 p-8 w-96 text-center shadow-[0_0_30px_rgba(220,38,38,0.4)]">
                <h2 className="text-red-500 text-3xl mb-2 font-mono uppercase tracking-widest animate-pulse">
                    ARHIVA IZGUBLJENA
                </h2>
                <p className="text-[#b87333] text-xs mb-8 font-mono">
                    (TIMELINE CORRUPTED // SIGNAL LOST)
                </p>
                
                <button 
                    onClick={handleRestart}
                    className="w-full bg-red-900 hover:bg-[#39ff14] hover:text-black text-white font-mono font-bold py-3 px-4 transition-colors border-2 border-red-700 hover:border-[#39ff14] uppercase tracking-wider cursor-pointer"
                >
                    Premotaj Traku (Restart)
                </button>
            </div>
        </div>
      )}

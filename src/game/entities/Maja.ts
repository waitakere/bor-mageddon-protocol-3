...
export class Maja extends Phaser.Physics.Arcade.Sprite {
    ...
    private executeAction(action: string) {
        if (action === 'special') {
            if (this.smfMeter >= 25) { this.executeBalkanSuplex(); return; }
            action = 'punch-2';
        }
        if (action === 'finisher') {
            if (this.smfMeter >= 100) { this.executeIndustrialDrill(); return; }
            action = 'kick-2';
        }
    ...
    private executeBalkanSuplex() {
        ...
        this.scene.physics.overlap(grabZone, (this.scene as any).enemies, (gz, enemy: any) => {
            if (!grabbedEnemy && !enemy.isDead) grabbedEnemy = enemy;
        });

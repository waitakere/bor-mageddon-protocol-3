...
export class Darko extends Phaser.Physics.Arcade.Sprite {
    ...
    private executeAction(action: string) {
        if (action === 'special') {
            if (this.smfMeter >= 25) { this.executeRoundhouseSpin(); return; }
            action = 'kick-1';
        }
        if (action === 'finisher') {
            if (this.smfMeter >= 100) { this.executeGuitarRiff(); return; }
            action = 'punch-2';
        }
    ...
    private executeRoundhouseSpin() {
        ...
        this.scene.physics.add.overlap(spinZone, (this.scene as any).enemies, (sz, enemy: any) => {
            if (enemy.takeDamage) {
                ...
                const pushDir = enemy.x > this.x ? 1 : -1;
                if (enemy.body) enemy.setVelocityX(300 * pushDir); // Pushes hard
            }
        });
    ...
    private executeGuitarRiff() {
        ...
        // Distorted room effect
        this.scene.cameras.main.shake(800, 0.015);
        this.scene.cameras.main.flash(300, 0, 255, 255); // Cyan flash

        // Room-wide electricity damage
        const enemies = (this.scene as any).enemies.getChildren();
        enemies.forEach((enemy: any) => {
            if (!enemy.isDead && enemy.takeDamage) {
                enemy.takeDamage(100); // Shock damage
                enemy.setTint(0x00ffff);
                this.scene.time.delayedCall(200, () => enemy.clearTint());
            }
        });
